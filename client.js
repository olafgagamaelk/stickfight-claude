...
const WEAPON_VISUALS = {
  fists:   {name:'Næver', type:'melee', color:'#dfe3e8'},
  bat:     {name:'Bat', type:'melee', color:'#c98a4b'},
  pistol:  {name:'Pistol', type:'ranged', color:'#9ad1ff'},
  smg:     {name:'Maskinpistol', type:'ranged', color:'#ffd166'},
  shotgun: {name:'Haglgevær', type:'ranged', color:'#ff8c42'},
  sniper:  {name:'Riffel', type:'ranged', color:'#8be38b'},
  rocket:  {name:'Raketkaster', type:'ranged', color:'#ff5566'}
};
  ...
  snap.players.forEach(sp=>{
    ...
    if(!lp){
      lp = { id:sp.id, idx:sp.idx, name:sp.name, color:sp.color,
        x:sp.x, y:sp.y, prevX:sp.x, prevY:sp.y, targetX:sp.x, targetY:sp.y, recvTime:now,
        facing:sp.facing, grounded:sp.grounded, weapon:sp.weapon, ammo:sp.ammo, damage:sp.damage,
        alive:sp.alive, ragdollTimer:sp.ragdollTimer, walkPhase:sp.walkPhase, roundWins:sp.roundWins,
        aimUp:sp.aimUp, aimDown:sp.aimDown, invuln:sp.invuln, hitFlash:0, attackFlash:0 };
      initSkeleton(lp);
      ...
      lp.facing=sp.facing; lp.grounded=sp.grounded; lp.weapon=sp.weapon; lp.ammo=sp.ammo;
      lp.damage=sp.damage; lp.alive=sp.alive; lp.ragdollTimer=sp.ragdollTimer;
      lp.walkPhase=sp.walkPhase; lp.roundWins=sp.roundWins; lp.aimUp=sp.aimUp; lp.aimDown=sp.aimDown; lp.invuln=sp.invuln;
    }
  });
...
function processEvents(events){
  for(const e of events){
    if(e.t==='hit'){
      ...
      if(lp){ ragdollImpulse(lp, e.dirx, e.diry, e.power); lp.hitFlash=0.15; spawnParticles(lp.x, lp.y-30, 10, '#fff', 260, 0.35, 900); }
      sfx.hit();
    } else if(e.t==='ko'){
      ...
      const lp = netPlayers.get(e.id);
      const col = WEAPON_VISUALS[e.weapon] ? WEAPON_VISUALS[e.weapon].color : '#ffe8a3';
...
/* ============================================================
   MENU / LOBBY UI
   ============================================================ */
const screens = { menu:document.getElementById('screen-menu'), howto:document.getElementById('screen-howto'), lobby:document.getElementById('screen-lobby') };
function showScreen(name){
  Object.values(screens).forEach(s=>s.classList.add('hidden'));
  if(name) screens[name].classList.remove('hidden');
}
function showMenuError(msg){ document.getElementById('menuError').textContent = msg||''; }

document.getElementById('btn-create').onclick = ()=>{
  ensureAudio(); showMenuError('');
  if(!ws || ws.readyState!==1){ connect(); ws.addEventListener('open', ()=> send({t:'create', name:document.getElementById('nameInput').value}), {once:true}); }
  else send({t:'create', name:document.getElementById('nameInput').value});
};
document.getElementById('btn-join').onclick = ()=>{
  ensureAudio(); showMenuError('');
  const code = document.getElementById('codeInput').value.trim();
  if(code.length<4){ showMenuError('Indtast en 4-tegns kode.'); return; }
  if(!ws || ws.readyState!==1){ connect(); ws.addEventListener('open', ()=> send({t:'join', code, name:document.getElementById('nameInput').value}), {once:true}); }
  else send({t:'join', code, name:document.getElementById('nameInput').value});
};
...
function frame(now){
  ...
  if(latestSnap && (uiState==='GAME')){
    ...
    // interpolate + advance cosmetic ragdolls
